...
struct Args {
    ...
    /// Override network (testnet | mainnet)
    #[arg(long)]
...
impl MintConfig {
    ...
    fn validate(&self) -> Result<()> {
        ...
        if self.network.network_type != "testnet" && self.network.network_type != "mainnet" {
            anyhow::bail!("network.network_type must be 'testnet' or 'mainnet'");
        }
        Ok(())
    }
}
