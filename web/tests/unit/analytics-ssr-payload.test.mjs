...
test('analytics server load keeps the initial SSR relay payload small', () => {
  ...
  assert.match(source, /fetchRecentMarkets\(ANALYTICS_MARKET_LIMIT, \{ edition \}\)/);
  assert.match(source, /fetchRecentDiscussions\(ANALYTICS_DISCUSSION_LIMIT, \{ edition \}\)/);
  assert.match(source, /fetchRecentTrades\(ANALYTICS_TRADE_LIMIT, \{ edition \}\)/);
