...
export async function getMarketKeysets(
  ...
  const cacheKey = `${market.eventId}:${mintInfo.url}`
  ...
  const keysets = await fetchKeysetsFromMint(mintInfo.url, market, ttl)
...
export function hasKeysetsCached(market: Market, mintUrl: string): boolean {
  const cacheKey = `${market.eventId}:${mintUrl}`
  const cached = keysetCache.get(cacheKey)
...
interface ScopedMintKeysResponse {
  market_id: string
  long_keyset?: ScopedKeysetInfo
  short_keyset?: ScopedKeysetInfo
}
...
/**
 * Fetch keysets from the market-scoped GET /{event_id}/v1/keys endpoint.
 * Falls back to the legacy flat endpoint during migration.
 */
async function fetchKeysetsFromMint(
  ...
  try {
    const scopedResponse = await fetch(
      `${mintUrl}/${encodeURIComponent(market.eventId)}/v1/keys`,
      {
        method: 'GET',
    ...
    if (scopedResponse.ok) {
      const data = await scopedResponse.json() as ScopedMintKeysResponse
      const longKeyset = toScopedKeyset(data.long_keyset, mintUrl)
      const shortKeyset = toScopedKeyset(data.short_keyset, mintUrl)
      const now = Math.floor(Date.now() / 1000)
...
function toScopedKeyset(info: ScopedKeysetInfo | undefined, mintUrl: string): Keyset | null {
  if (!info?.id) return null

  const publicKey =
    info.keys?.['1']?.pubkey ??
    Object.values(info.keys ?? {})[0]?.pubkey ??
    ''
