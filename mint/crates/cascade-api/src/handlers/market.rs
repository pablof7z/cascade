...
pub async fn create_market(
        ...
        .await
    {
        ...
        Err(e) => {
            tracing::error!(
                "Failed to create LONG keyset for market {}: {}",
                req.slug,
                e
            );
            return (
        ...
        .await
    {
        ...
        Err(e) => {
            tracing::error!(
                "Failed to create SHORT keyset for market {}: {}",
                req.slug,
                e
            );
            return (
    ...
    {
        Ok(market) => {
            if let Err(e) = state.db.insert_market(&market).await {
                tracing::error!(
                    "Failed to persist market {} (event_id={}): {}",
                    market.slug,
                    market.event_id,
                    e
                );
