...
test('charts tab renders an inline SVG time-series price chart', () => {
  ...
  assert.match(source, /<svg[\s\S]*viewBox=\{`0 0 \$\{chartWidth\} \$\{chartHeight\}`\}/);
  assert.match(source, /<polyline[\s\S]*points=\{chartPolylinePoints\}/);
  ...
  assert.match(source, /formatRelativeTime\(chartStartTrade\.createdAt\)[\s\S]*formatRelativeTime\(chartEndTrade\.createdAt\)/);
...
test('market surface presents prediction market copy as LONG/SHORT while preserving internal long/short data model', () => {
  ...
  assert.match(source, /<span[^>]*>LONG<\/span>/);
  assert.match(source, /<span[^>]*>LONG<\/span>[\s\S]*<strong[^>]*>\{priceCents\(impliedProbability\)\}<\/strong>/);
  assert.match(source, /<span[^>]*>SHORT<\/span>[\s\S]*<strong[^>]*>\{priceCents\(oppositeProbability\)\}<\/strong>/);
