<script lang="ts">
  ...
  let type = $state<'argument' | 'evidence' | 'rebuttal' | 'analysis'>('analysis');
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      await publishMarketPost(title, body, stance, type, marketEventId, marketCreatorPubkey);
      content = '';
      stance = 'neutral';
      type = 'analysis';
      onPostPublished?.();
...
<div class="mt-6 pt-6 border-t border-neutral-800">
  ...
  <form onsubmit={(e) => { e.preventDefault(); handleSubmit(); }} class="space-y-4">
    ...
    <div class="flex gap-4">
      <!-- Stance dropdown -->
      <div class="flex-1">
        <label class="block text-xs text-neutral-500 mb-1">Stance</label>
        <select
          bind:value={stance}
          disabled={publishing}
