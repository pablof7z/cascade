...
use crate::types::{
    BuyRequest, EscrowStatsResponse, InvoiceStatusRequest, LightningSellRequest,
    LightningTradeRequest, SellRequest, SettleRequest, TokenOutput, TradeResponse,
};
use axum::{
...
use cdk::nuts::{BlindedMessage, Proof, Proofs, SwapRequest};
use cdk::util::hex;
use cdk::Amount;
...
pub async fn buy(
    ...
    match executor.execute_buy(&market, side, req.quantity, req.buyer_pubkey) {
        Ok(trade) => {
            ...
            if let Ok((price_long, price_short)) = state
                .market_manager
                .lmsr()
                .get_prices(new_q_long, new_q_short)
            {
                if let Err(e) = state
                    .db
                    .insert_lmsr_snapshot(
                        &market.slug,
                        new_q_long,
                        new_q_short,
                        price_long,
                ...
                {
                    tracing::warn!(
                        "Failed to record LMSR snapshot for market {}: {}",
                        market.slug,
                        e
                    );
                }
                ...
                let blinded_messages: Vec<BlindedMessage> = match req
                    ...
                    .map(|b| {
                        ...
                        let blinded_secret_bytes =
                            hex::decode(&b.b_).map_err(|e| format!("Invalid B_ hex: {}", e))?;
                        let blinded_secret =
                            cdk::nuts::PublicKey::from_slice(&blinded_secret_bytes)
    ...
    match executor.execute_sell(&market, side, req.quantity, req.seller_pubkey) {
        Ok(trade) => {
            ...
            let tokens: Vec<TokenOutput> = if !req.proofs.is_empty() && !req.outputs.is_empty() {
                ...
                let input_proofs: Proofs = match req
                    .proofs
                    .iter()
                    .map(|p| {
...
pub async fn create_lightning_trade(
    ...
    let trade = match executor.execute_buy(
        &market,
        side,
        req.amount_sats as f64,
    ...
    let lightning_order = match invoice_service
        .create_lightning_order(
            &req.market_id,
            &market.event_id,
            &req.side,
            trade.cost_sats,
...
pub async fn sell_lightning_trade(
    ...
    let trade = match executor.execute_sell(
        &market,
        side,
        req.amount_sats as f64,
...
pub async fn settle_lightning_trade(
    ...
    match invoice_service
        .settle_by_order_id(&order_id, &preimage)
        .await
    {
        Ok(_) => (
            StatusCode::OK,
