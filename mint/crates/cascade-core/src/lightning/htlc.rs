...
use super::types::{HtlcInfo, PaymentHash, PaymentStatus, Preimage};
...
pub enum HtlcCondition {
    ...
    HashTimeLock { hash: PaymentHash, cltv_expiry: u64 },
}
...
impl HtlcState {
    ...
    pub fn new_hash_lock(payment_hash: PaymentHash, amount_msat: u64, expiry_seconds: u64) -> Self {
    ...
    pub fn claim(&mut self, witness: &HtlcWitness) -> Result<()> {
        ...
        if self.status == PaymentStatus::Settled {
            return Err(CascadeError::invalid_input(
                "HTLC already settled".to_string(),
            ));
        }
        if self.status == PaymentStatus::Refunded {
            return Err(CascadeError::invalid_input(
                "HTLC already refunded".to_string(),
            ));
        }
        ...
        if self.is_expired() {
            return Err(CascadeError::invalid_input(
                "HTLC has expired, must be refunded".to_string(),
            ));
        }
        ...
        if !witness.verify(&self.condition) {
            return Err(CascadeError::invalid_input(
                "Invalid HTLC witness".to_string(),
            ));
        }
    ...
    pub fn refund(&mut self) -> Result<()> {
        ...
        if self.status == PaymentStatus::Settled {
            return Err(CascadeError::invalid_input(
                "Cannot refund settled HTLC".to_string(),
            ));
        }
        ...
        if !self.is_expired() {
            return Err(CascadeError::invalid_input(
                "HTLC not yet expired".to_string(),
            ));
        }
    ...
    pub fn fail(&mut self) -> Result<()> {
        if self.status == PaymentStatus::Settled {
            return Err(CascadeError::invalid_input(
                "Cannot fail settled HTLC".to_string(),
            ));
        }
...
impl HtlcHandler {
    ...
    pub fn create_htlc(
        ...
        let state = match cltv_expiry {
            ...
            None => {
                HtlcState::new_hash_lock(payment_hash.clone(), amount_msat, self.default_expiry)
            }
        };
    ...
    pub fn cleanup(&mut self, older_than: i64) {
        self.htlcs.retain(|_, state| match state.status {
            PaymentStatus::Settled => state.settled_at.map(|t| t > older_than).unwrap_or(true),
            PaymentStatus::Refunded => state.refunded_at.map(|t| t > older_than).unwrap_or(true),
            _ => true,
        });
    }
}
