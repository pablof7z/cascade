...
async fn test_resolution_long_wins() {
    ...
    let resolve_result = market_manager.resolve_market(&event_id, Side::Long).await;
    assert!(
    ...
    assert!(resolved_market.is_resolved(), "Market should be resolved");
}
...
async fn test_resolution_short_wins() {
    ...
    assert!(
        result.is_ok(),
        "Market should resolve to SHORT: {:?}",
        result.err()
    );
...
async fn test_cannot_resolve_twice() {
    ...
    let first_result = market_manager.resolve_market(&event_id, Side::Long).await;
    assert!(
        first_result.is_ok(),
        "First resolution should succeed: {:?}",
        first_result.err()
    ...
    let second_result = market_manager.resolve_market(&event_id, Side::Short).await;
    assert!(
