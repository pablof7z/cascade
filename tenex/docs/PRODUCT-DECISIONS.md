...
### 2.10 Kind 983 Trade / Settlement Events
> "Trade events use kind 983, market events use kind 982"

- Kind 983 is the trade / settlement event kind.
- Trade events use kind 983.
- Market events use kind 982.

### 2.11 NIP-60 Wallet Compliance
> "Cashu wallet must follow NIP-60 spec"
> "browser-local wallet proofs are the canonical position store"

- Cashu wallet must follow NIP-60 spec.
- Browser-local wallet proofs are the canonical position store.

### 2.12 Standard CDK Routes Only
> "Use standard Cashu CDK routes for all mint operations"
> "No custom/legacy mint routes -- if a route doesn't follow CDK spec, remove it"

- Use standard Cashu CDK routes for all mint operations.
- Use melt routes for Lightning settlement.
- Remove custom or legacy mint routes that do not follow the CDK spec.
...
### 3.7 No Resolution Language — Ever
...
- NEVER use "resolution", "resolves", "resolved", "market closes", or "winner payout".
- ALWAYS use "mint", "withdraw", "withdrawal proceeds", "exit position", and "LMSR price at withdrawal time".
- Markets never close, never resolve, and never declare winners.
...
### 6.10 Self-Initiate Work + Publish Status
> "I shouldn't have to babysit you on obvious shit like this."
> "Don't describe -- act."

- Agents should self-initiate work instead of waiting for explicit instruction.
- Agents must publish status updates to Nostr via the project status document.
- When the next step is obvious, act instead of narrating the intent.
...
### 9.4 Prediction Leagues (Planned)
> "Prediction leagues are a planned feature"

- Prediction leagues are a planned feature.
- The concept is competitive group predictions.
- Not yet implemented, but the directive stands.
...
| Date | Changes |
|------|---------|
| 2026-04-13 10:20 | Restored `tenex/docs/PRODUCT-DECISIONS.md` from `a64c04c`. Added: §2.10 Kind 983 trade / settlement events, §2.11 NIP-60 wallet compliance, §2.12 Standard CDK routes only, §3.7 no resolution language, §6.10 self-initiate work + publish status, §9.4 prediction leagues. |
