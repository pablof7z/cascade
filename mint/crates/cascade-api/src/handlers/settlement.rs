...
use crate::types::{
    BlindedMessageInput, ErrorResponse, KeysetPublicKeyResponse, MarketKeysetResponse,
    MarketMintKeysResponse, MarketRedeemRequest, MarketRedeemResponse, MarketSettleRequest,
    MarketSettleResponse, ProofInput, TokenOutput,
};
use axum::{
    extract::{Path, State},
    http::StatusCode,
...
use cascade_core::{lmsr::LmsrEngine, market::Side};
use cdk::nuts::{BlindedMessage, Id, Proof, Proofs, PublicKey, SwapRequest};
...
fn proof_input_to_cdk_proof(proof: &ProofInput) -> Result<Proof, String> {
    ...
    // Parse keyset ID
    let keyset_id = Id::from_str(&proof.id).map_err(|e| format!("Invalid keyset ID: {}", e))?;

    // Parse secret
    let secret = Secret::new(proof.secret.clone());

    // Parse C (commitment) from hex to public key
    let c_bytes = hex::decode(&proof.C).map_err(|e| format!("Invalid commitment hex: {}", e))?;
...
fn blinded_msg_input_to_cdk(b: &BlindedMessageInput) -> Result<BlindedMessage, String> {
    ...
    let keyset_id =
        Id::from_str(&b.id).map_err(|e| format!("Invalid keyset ID '{}': {}", b.id, e))?;
    let blinded_secret_bytes = hex::decode(&b.b_).map_err(|e| format!("Invalid B_ hex: {}", e))?;
...
fn validate_proof(proof: &ProofInput) -> Result<(), String> {
    ...
    if let Some(dleq) = &proof.dleq {
        ...
        if !dleq.e.chars().all(|c| c.is_ascii_hexdigit())
            || !dleq.s.chars().all(|c| c.is_ascii_hexdigit())
            || !dleq.r.chars().all(|c| c.is_ascii_hexdigit())
        {
...
pub async fn redeem(
    ...
    StatusCode,
    Json<Result<MarketRedeemResponse, ErrorResponse>>,
) {
    ...
    let gross_payout =
        match calculate_sell_refund(lmsr, market.q_long, market.q_short, side, req.shares) {
            Ok(refund) => refund,
            Err(msg) => {
                return (
                    StatusCode::BAD_REQUEST,
...
pub async fn settle(
    ...
    StatusCode,
    Json<Result<MarketSettleResponse, ErrorResponse>>,
) {
    // Parse side
    let side = match req.side.to_lowercase().as_str() {
        "yes" => Side::Long,
    ...
    let tokens: Vec<TokenOutput> = if won && net_payout > 0 {
        let blinded_messages: Vec<BlindedMessage> = match req
            .outputs
            .iter()
            .map(blinded_msg_input_to_cdk)
            .collect::<Result<Vec<_>, _>>()
        ...
        if blinded_messages.is_empty() {
            return (
                ...
                Json(Err(ErrorResponse {
                    ...
                    details: Some(
                        "Winners must provide blinded messages for SAT payout".to_string(),
                    ),
                })),
            );
        }
