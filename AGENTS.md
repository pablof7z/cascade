...
- **Typography**: Headings use `font-heading` (Instrument Serif). Body/UI use `font-sans` (Inter). Numbers/data use `font-mono` (JetBrains Mono). `text-sm font-medium` for interactive elements, `text-xs` for metadata. No uppercase tracking except explicit label categories.
