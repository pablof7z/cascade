...
async fn test_buy_sell_lifecycle() {
    ...
    assert!(buy_result.is_ok(), "Buy should succeed: {:?}", buy_result.err());
    ...
    let _sell_result =
        executor.execute_sell(&updated_market, Side::Long, trade.quantity, "trader123".to_string());
...
async fn test_trade_sides() {
    ...
    assert!(long_result.is_ok(), "LONG buy should succeed: {:?}", long_result.err());
