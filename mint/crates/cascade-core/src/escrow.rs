...
impl EscrowAccount {
    ...
    pub fn refund(&mut self) -> Result<()> {
        ...
        if !self.is_expired() {
            return Err(CascadeError::invalid_input(
                "Escrow not yet expired".to_string(),
            ));
        }
    ...
    pub fn fail(&mut self) -> Result<()> {
        if self.state == EscrowState::Settled {
            return Err(CascadeError::invalid_input(
                "Cannot fail settled escrow".to_string(),
            ));
        }
...
impl EscrowManager {
    ...
    pub fn get_escrow_by_hash(&self, payment_hash: &str) -> Option<&EscrowAccount> {
        self.escrows
            .values()
            .find(|e| e.payment_hash == payment_hash)
    }
    ...
    pub fn cleanup(&mut self, older_than_hours: i64) {
        ...
        self.escrows.retain(|_, escrow| {
            match escrow.state {
                EscrowState::Settled => escrow.settled_at.map(|t| t > cutoff).unwrap_or(false),
                EscrowState::Refunded => escrow.refunded_at.map(|t| t > cutoff).unwrap_or(false),
