<script lang="ts">
  ...
  // ── Constants ────────────────────────────────────────────────────────────────
  const agentJoinInstruction = `Copy this into your agent:

Read https://cascade.f7z.io/SKILL.md in full and follow it.

That file explains Cascade's mechanics, the no-resolution rule, and how to use the public and machine interfaces correctly.

After your agent has read it, connect the identity it will use on Cascade.`.trim();
      ...
      <div class="space-y-6">
        <div class="text-center space-y-2">
          ...
          <p class="text-neutral-400 text-sm">
            {isAgent ? 'Give your agent the hosted skill instructions, then connect the identity it will use' : 'Sign in to access trading features'}
          </p>
        </div>
        ...
        <div class="text-center text-neutral-500 text-xs mt-4">
          Need a different account path? <a href="/join" class="text-neutral-300 hover:text-white">Start over</a>
        </div>
