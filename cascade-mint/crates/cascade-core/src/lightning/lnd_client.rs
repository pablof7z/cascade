...
use crate::lightning::types::{
    InvoiceState, LightningInvoice, PaymentHash, Preimage,
};
use std::sync::Arc;
...
impl LndClient {
    ...
    pub async fn get_invoice(&self, _payment_hash: &PaymentHash) -> Result<Option<LightningInvoice>> {
        if !self.connected {
            return Err(CascadeError::PaymentError("Not connected to LND".to_string()));
        }
    ...
    pub async fn wait_for_settlement(
        ...
        Err(CascadeError::PaymentError("Invoice settlement timeout".to_string()))
    }
...
mod tests {
    ...
    fn test_lnd_config_with_macaroon() {
        ...
        assert_eq!(config.macaroon_path.as_ref().unwrap(), "/path/to/admin.macaroon");
    }
    ...
    async fn test_lnd_client_add_invoice_when_connected() {
        ...
        let invoice = client.add_invoice(
            100_000, // 100k msat
    ...
    async fn test_lnd_client_verify_preimage_valid() {
        ...
        let is_valid = client.verify_preimage(&preimage, &payment_hash).await.unwrap();
        assert!(is_valid);
    }
    ...
    async fn test_lnd_client_verify_preimage_invalid() {
        ...
        let is_valid = client.verify_preimage(&preimage, &wrong_hash).await.unwrap();
        assert!(!is_valid);
    }
    ...
    async fn test_invoice_creation_and_verification_flow() {
        ...
        let is_valid = client.verify_preimage(&payment_preimage, &invoice.payment_hash).await.unwrap();
        assert!(!is_valid);
    }
}
