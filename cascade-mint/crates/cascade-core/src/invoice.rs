...
impl InvoiceService {
    ...
    pub async fn generate_trade_invoice(
        ...
        let invoice = self.lnd_client.add_invoice(
            amount_msat,
            description.or_else(|| Some(format!("Cascade: {} {} for {}", side, amount_sats, market_slug))),
    ...
    pub async fn verify_and_settle(
        ...
        // Settle escrow
        let _escrow = self.escrow_manager.settle_by_payment_hash(
            &payment_hash.to_hex(),
            &preimage,
        )?;
        ...
        Err(CascadeError::invalid_input("Order lookup not implemented".to_string()))
    }
    ...
    pub async fn create_lightning_order(
        ...
        let invoice = self.lnd_client.add_invoice(
            amount_msat,
            Some(format!("Cascade: {} {} for {}", side, amount_sats, market_slug)),
            Some(self.default_expiry_seconds),
            Some(self.cltv_delta),
        ).await?;
        ...
        // Store the order
        self.orders.insert(order_id_to_use.clone(), final_order.clone());
        self.orders_by_payment_hash.insert(payment_hash_hex, order_id_to_use);
    ...
    pub async fn get_invoice_status(&self, payment_hash: &str) -> Result<String> {
        ...
        match PaymentHash::from_hex(payment_hash) {
            ...
            Err(_) => Err(CascadeError::invalid_input("Invalid payment hash".to_string())),
        }
    }
    ...
    pub async fn settle_by_order_id(
        ...
        if order.state == OrderState::Fulfilled {
            return Err(CascadeError::invalid_input("Order already fulfilled".to_string()));
        }
        if order.state == OrderState::Expired {
            return Err(CascadeError::invalid_input("Order has expired".to_string()));
        }
        if order.state == OrderState::Cancelled {
            return Err(CascadeError::invalid_input("Order was cancelled".to_string()));
        }

        // Settle the escrow
        self.escrow_manager.settle_by_payment_hash(
            &order.payment_hash,
            preimage,
        )?;
