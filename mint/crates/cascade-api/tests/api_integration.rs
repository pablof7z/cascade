...
#[tokio::test]
async fn test_lightning_topup_quote_and_settlement_flow() {
    let url = create_product_test_server().await;
    let client = reqwest::Client::new();
    ...
    let settle_response = client
        .post(format!(
            "{url}/api/wallet/topups/lightning/{quote_id}/settle"
        ))
        .send()
        .await
        .unwrap();
    assert_eq!(settle_response.status(), 200);
    let settle_payload: serde_json::Value = settle_response.json().await.unwrap();
    assert_eq!(settle_payload["topup"]["status"].as_str(), Some("complete"));
    assert_eq!(
        settle_payload["wallet"]["available_minor"].as_u64(),
        Some(2500)
    );
    assert_eq!(settle_payload["wallet"]["pending_minor"].as_u64(), Some(0));
    assert_eq!(
        settle_payload["wallet"]["funding_events"][0]["rail"].as_str(),
        Some("lightning")
    );
}
